...
module Dcmgr::Models
  ...
  class IpLease < BaseNew
    ...
    def self.get_lease_address(network, from_ipaddr, to_ipaddr, order)
      ...
      raise ArgumentError unless from_ipaddr.is_a?(0 <= from_ipaddr && from_ipaddr <= 0xFFFFFFFF)
      raise ArgumentError unless to_ipaddr.is_a?(0 <= to_ipaddr && to_ipaddr <= 0xFFFFFFFF)
